...
- `references/serenity_framework.md` — distilled 12-model investment framework plus WOOK98 supplemental principles and a reusable 15-step checklist (merged from W-Y-P build).
- `references/case_patterns.md` — concrete bottleneck archetypes (AXTI/InP, SIVE/CPO laser, SOI, AAOI, XFAB, 800V power, A-share adaptation, neocloud) for pattern-matching new candidates (merged from W-Y-P build).
- `references/achievements_and_sources.md` — reliability labels and caveat text for public Serenity performance/identity claims (merged from W-Y-P build).
- `references/source_notes.md` — evidence source map and snapshot notes (merged from W-Y-P build).
- `references/update_2026_06.md` — incremental evidence-routing notes (merged from W-Y-P build).
- `references/serenity-methodology-operational.md` — **operational engine** (Chinese): hard valuation thresholds (§3.3), 14-point bottleneck criteria with tweet-id citations (§2.4), 10-item red-flag scan (§2.5), thesis state machine for entry/sizing/exit (§5), decision-neutral expected-return output (§3.5), microcap execution mechanics (§5.9), and a rigorous honesty/de-bias layer (§8: confirmation bias, PT-only-up, talking-his-book, microcap self-pump loop, LLM "confidently wrong" on multi-hop). Includes "Phase B 实测" guardrails from real runs (same-source/same-date multiple comparison, institutional-ownership threshold applies to small caps only). Load this for any concrete buy/sell judgment, valuation, or position-sizing question (merged from ZadAnthony build).
- `references/sources/serenity-corpus.csv` — **raw primary-source tweet corpus** (5,963 Serenity tweets, 2025-07-02..2026-06-06, ~96% English, with tweet_id/created_at/engagement metrics; merged from yijiashu build). LEAD-GENERATION ONLY per the evidence ladder: use it to see what Serenity actually discussed and how often, then verify every claim against primary filings/transcripts. Do NOT cite raw tweets as proof. Note: the source repo mislabeled the count as 44,514 and the handle as @stockgodserenity; actual rows are 5,963 and the corpus's cashtag profile ($NBIS/$SIVE/$AXTI/$AAOI) matches the known @aleabitoreddit Serenity universe.
...
- `scripts/query_corpus.py` — cross-platform reader for `references/sources/serenity-corpus.csv` (overview stats / top cashtags / `--ticker X` to pull raw tweets for a name as leads; merged from yijiashu build).
